...
class ADODB_mssqlnative extends ADOConnection {
	...
	{
		if($this->debug) ADOConnection::outp("<hr>CreateSequence2008($seq,$start)"); //***
	...
	{
		if($this->debug) ADOConnection::outp("<hr>GenID2008($seq,$start)"); //***
		sqlsrv_begin_transaction($this->_connectionID);
	...
	*/
	...
	function _connect($argHostname, $argUsername, $argPassword, $argDatabasename)
	{
		...
		$connectionInfoDebug = $connectionInfo; //***
		unset($connectionInfoDebug["UID"]); //***
		unset($connectionInfoDebug["PWD"]); //***
		if ($this->debug) ADOConnection::outp("<hr>Connecting... hostname: $argHostname; params: ".var_export($connectionInfoDebug,true)); //***
	...
	*/
	...
	// Use OFFSET and FETCH for SQL Server >= 2012 //***
	function SelectLimit($sql,$nrows=-1,$offset=-1,$inputarr=false,$secs2cache=0)
	{
		preg_match('/Microsoft SQL Server (\d+)/', $this->GetOne("SELECT @@version"), $m);
		$_hasOrderBy = @$inputarr["_hasOrderBy"];
		unset($inputarr["_hasOrderBy"]);
		if (intval(@$m[1]) >= 2012 && is_bool($_hasOrderBy)) { // Microsoft SQL Server >= 2012 and SQL with ORDER BY clause
			$hasOrderBy = $_hasOrderBy;
			$selectDistinct = preg_match('/(^\s*select\s+distinct\s+)/i', $sql);
			if (!$hasOrderBy && !$selectDistinct) {
					$sql .= " ORDER BY @@version";  // Add dummy ORDER BY clause
					$hasOrderBy = TRUE;
			}
			if ($hasOrderBy) {
				if ($offset > -1)
				...
				if ($secs2cache > 0)
					return $this->CacheExecute($secs2cache,$sql,$inputarr);
				else
	...
	function Execute($sql, $inputarr = false) { //***
		...
		if ($ret && $ret instanceof ADORecordset_mssqlnative && !$GLOBALS["ADODB_COUNTRECS"] &&
			preg_match('/^SELECT\\s/i', $sql) && // SELECT statement
			!preg_match('/\/\*SelectLimit\*\/$/', $sql)) { // Not using ADOConnection::SelectLimit
...
class ADORecordset_mssqlnative extends ADORecordSet {
	...
	function _initrs()
	{
		...
		//$this->_numOfRows = -1;//not supported // sqlsrv_num_rows() does not work //***
